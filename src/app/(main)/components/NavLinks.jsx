  ...
  const pathname = usePathname()
    ...
    <div className="flex flex-col right justify-between items-end gap-2">
      <div className="flex gap-1">
        <Link className={`nav-link ${pathname === "/grid"?"bg-primary-500":""}`} href={`/grid?${params?.toString()}`}>Grid</Link>
        <Link className={`nav-link ${pathname === "/"?"bg-primary-500":""}`} href={`/?${params?.toString()}`}>Map</Link>
      </div>
