  ...
  return (
    <div className="flex flex-col gap-0.5 mt-2 border-b pb-2">
      ...
      <aside className="flex gap-0.5">
        <button className={`${!filterOpen ? null : "bg-amber-400"} h-fit`} onClick={() => { setFilterOpen(!filterOpen); setOpenFilter(null) }}>Filters</button>
            ...
            {["Location", "Type", "Risk"].map((label) => {
              ...
              return (
                <div key={label} className="flex flex-wrap gap-0.5">
                  <div><button className="bg-amber-500" onClick={() => setOpenFilter(openFilter === label ? null : label)}>{label}</button></div>
                  <div className={`flex flex-wrap  gap-0.5  `}>
                    {filters[key].map((f, i) => (
                      <button key={i} onClick={() => multipleSelection(key, f)} className={selected[key].includes(f) ? "bg-amber-400" : ""}>{f}</button>
                    ))}
                  </div>
                </div>
              )
      ...
      <button className="w-fit active:bg-amber-400" onClick={() => {
