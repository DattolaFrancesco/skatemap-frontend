    ...
    return(
        <div className="w-full flex flex-wrap justify-between gap-2  py-2 border-t border-b ">
               <div className="flex flex-wrap gap-2">
                    <Link href={"/dashboard/"} className={`nav-link ${pathname === "/dashboard"?" bg-amber-400!":""}`}>MY SPOTS</Link>
                      {isAdmin && <Link href={"/dashboard/allSpot"} className={`nav-link ${pathname === "/dashboard/allSpot"?" bg-amber-400!":""}`}>ALL SPOTS</Link>}
                    <Link href={"/dashboard/favourites"} className={`nav-link ${pathname === "/dashboard/favourites"?" bg-amber-400!":""}`}>FAVOURITES</Link>
                    {isAdmin && 
                    <Link href={"/dashboard/requests"} className={`relative nav-link ${pathname === "/dashboard/requests"?" bg-amber-400!":""}`}>
                      REQUESTS
                   {pending > 0 && <div className="absolute rounded-full w-2 h-2 top-[-3] right-[-3] bg-red-500 text-[10px] animate-pulse"></div>}
                    </Link>}
                    {isSuperAdmin && <Link href={"/dashboard/users"} className={`nav-link ${pathname === "/dashboard/users"?" bg-amber-400!":""}`}>USERS</Link>}
                    {isSuperAdmin && <Link href={"/dashboard/chatBot"} className={`nav-link ${pathname === "/dashboard/chatBot"?" bg-amber-400!":""}`}>CHATBOT AI</Link>}
               </div>
               <div className="flex gap-2">
